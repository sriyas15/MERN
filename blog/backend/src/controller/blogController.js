...
export const getAllBlogs = asyncHandler(async (req, res) => {
        ...
        const getAll = await Blogs.find().populate({
                path: "author", select: "name username avatar followers following bio",
                populate: [
                {
                path: "followers",
                ...
                .populate({ path: "likes", select: "name username avatar _id followers following" })
...
export const toggleLike = asyncHandler(async (req, res) => {
  ...
  const updatedBlog = await Blogs.findByIdAndUpdate(
    ...
    isLiked
      ? { $pull: { likes: userId } }
      : { $push: { likes: userId } },
    { new: true, timestamps: false }  // ✅ prevents updatedAt from changing
  );

  res.status(200).json({
    message: isLiked ? "Unliked the post" : "Liked",
